<script lang="ts">    
    ...
    import type { RowSettings_v1 } from "./RowSettings";
    import type { WidgetContext } from "$lib/types/Widgets";
    import { findWidget } from "..";
    
    export let settings: RowSettings_v1;
...
{#if context.editing}
    <div 
        class="flex flex-row justify-stretch flex-1 border border-dashed rounded-md p-2 min-h-[20px] min-w-[20px]"
...
{:else} 
    <div class="flex flex-row justify-stretch flex-1">
        {#each (settings.children ?? []) as child(child.id)}
            <svelte:component this={findWidget(context.widgets[child.id].type)} settings={context.widgets[child.id]} {context} />
        {/each}
    </div> 
